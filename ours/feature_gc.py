...
def get_error_img_path_list():
    ...
    for row_i, row in record_df.iterrows():
        ...
        error_img_path_list.append(os.path.join("/home/mml/workspace/ultralytics/",img_file_path))
    return error_img_path_list
...
def case_study():
    ...
    for row_i,row in epoch_0_df.iterrows():
        image_path = row["image_path"]
        sample_idx = row["sample_idx"]
        imgPath_to_sampleId[image_path] = sample_idx

    error_sample_idx_list = []
    for error_label_file in error_label_file_list:
        ...
        error_sample_idx_list.append(imgPath_to_sampleId[error_img_file])

    img_file = cases["class_error_case"]["img_file"]
    sample_idx = imgPath_to_sampleId[img_file]
    error_metric_over_epoch = metric_over_epoch_df.iloc[sample_idx, 1:].values

    metric_over_epoch = metric_over_epoch_df.iloc[:, 1:].values # (nums,features)

    # 过滤掉指定行
    filtered_data = np.delete(metric_over_epoch, error_sample_idx_list, axis=0)
    # 计算过滤后每列的均值
